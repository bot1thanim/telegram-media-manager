...
from app.telegram.keyboards import CB, recycle_bin_item_keyboard
...
async def list_recycle_bin(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    async with get_session() as session:
        ...
        result = await session.execute(
            select(Media)
            .where(Media.status == MediaStatus.DELETED.value)
        ...
        await query.edit_message_text(
            caption, reply_markup=recycle_bin_item_keyboard(media.id)
        )


async def restore_media_handler(
...
def register_recycle_bin_handlers(application) -> None:
    application.add_handler(
        CallbackQueryHandler(list_recycle_bin, pattern=f"^{CB.RECYCLE_BIN}$")
    )
    application.add_handler(
        CallbackQueryHandler(restore_media_handler, pattern=f"^{CB.REC_RESTORE}")
    )
    application.add_handler(
        CallbackQueryHandler(perm_delete_handler, pattern=f"^{CB.REC_PERM_DEL}")
    )
